...
class _adminDashboardState extends State<adminDashboard> {
  ...
  fetchUserAdmin()async{
    ...
    Timer.periodic(const Duration(seconds: 1), (Timer timer) {
      setState(() {
        admin = api.getAdminUser("user/", prefs.getString('userId'));
      });
    });
  }
                      ...
                      child: ListView(
                        ...
                        children: <Widget>[
                          ...
                          ListTile(
                            ...
                            onTap: () async{
                              SharedPreferences prefs = await SharedPreferences.getInstance();
                              var response = await api.postLogout('auth/logout', prefs.getString('token'));
                              if (response != 'Error logging out'){
                                prefs.remove('token');
                                prefs.remove('userId');
